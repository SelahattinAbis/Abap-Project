CLASS zabap010_sa_data_generator DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZABAP010_SA_DATA_GENERATOR IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.
    DATA roomreservations TYPE STANDARD TABLE OF zraph_sa_roomrsv WITH DEFAULT KEY.
    " Clear personal travel table and fill it from /DMO/ reference table
    DELETE FROM zraph_sa_travel.
    INSERT zraph_sa_travel FROM ( SELECT * FROM /dmo/a_travel_d ).
    out->write( 'Travel data copied.' ).

    " Clear personal booking table and fill it from /DMO/ reference table
    DELETE FROM zraph_sa_booking.
    INSERT zraph_sa_booking FROM ( SELECT * FROM /dmo/a_booking_d ).
    out->write( 'Booking data copied.' ).

    " Clear personal booking supplement table and fill it from /DMO/ reference table
    DELETE FROM zraph_sa_booksup.
    INSERT zraph_sa_booksup FROM ( SELECT * FROM /dmo/a_bksuppl_d ).
    ...
    IF hotel_count = 0.
        ...
        RETURN.
    ENDIF.
    DELETE FROM zraph_sa_roomrsv.
    SELECT travel_uuid, begin_date, end_date, total_price, currency_code
        FROM zraph_sa_travel INTO TABLE @DATA(travels).
    ...
    LOOP AT travels ASSIGNING FIELD-SYMBOL(<travel>).
        ...
        IF index MOD 4 <= 2.
            ...
            ENDIF.
        ENDLOOP.
        INSERT zraph_sa_roomrsv FROM TABLE @roomreservations.
